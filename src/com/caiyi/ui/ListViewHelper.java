...
public class ListViewHelper {
	...
	public static int getScrollY(AbsListView lv) throws NotSupportException {
		try {
			if (mMethod == null) {
				mMethod = Class.forName(AbsListView.class.getName()).getDeclaredMethod("computeVerticalScrollOffset");
				mMethod.setAccessible(true);
			}
			return (Integer) mMethod.invoke(lv);
		} catch (Exception e) {
	...
	public static int getScrollRange(AbsListView lv) throws NotSupportException {
		try {
			Method m = Class.forName(AbsListView.class.getName()).getDeclaredMethod("computeVerticalScrollRange");
			m.setAccessible(true);
			return (Integer) m.invoke(lv);
		} catch (Exception e) {
			throw new NotSupportException("invoke AbsListView method computeVerticalScrollOffset() failed !\n"
					+ e.getMessage(), e);
		}
